# The prompt
# PROMPT='$(_user_host)$(_python_venv)%{$fg[cyan]%}$(_current_dir) $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}➜ '
PROMPT='$(_user_host)%{$fg[cyan]%}$(_current_dir) $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}➜ '
...
function _user_host() {
  if [[ $(who am i) =~ \([-a-zA-Z0-9\.]+\) ]]; then
    # replace local macbook-pro to mac to make it short
    if [[ "$(uname -n)" =~ macbook-pro ]]; then
      me="%n@mac"
    else
      me="%n@%m"
    fi
